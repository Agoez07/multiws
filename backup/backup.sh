...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/Agoez07/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/Agoez07/backupuseryokko.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "hanzocan6@gmail.com" &> /dev/null
    git config --global user.name "Agoez07" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/Agoez07/backupuseryokko
    git push -f https://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@github.com/Agoez07/backupuseryokko.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/Agoez07/backupuseryokko/main/$NameUser/$NameUser.zip"
sleep 1
